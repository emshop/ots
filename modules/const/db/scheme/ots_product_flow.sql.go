...
const ots_product_flow=`CREATE TABLE  ots_product_flow (
		flow_id number(10)  not null AUTO_INCREMENT comment '流程编号' ,
		pl_id number(10)  not null  comment '产品线编号' ,
		parent_flow_id number(10) default 0 not null  comment '父级流程编号' ,
		...
		delay number(10) default 0 not null  comment '延后处理时长' ,
		timeout number(10)  not null  comment '超时时长' ,
		max_count number(10)  not null  comment '最大执行次数' ,
