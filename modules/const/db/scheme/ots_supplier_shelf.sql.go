...
const ots_supplier_shelf=`CREATE TABLE  ots_supplier_shelf (
		...
		invoice_type number(2) default 0 not null  comment '开票方式（0.不开发票）' ,
		spp_fee_discount number(10,5) default 0 not null  comment '商户佣金' ,
		...
		can_refund number(1) default 1 not null  comment '支持退货 (0.是,1.否)' ,
