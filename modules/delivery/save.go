...
func Save(deliveryID int64, source enums.ResultSource, resultCode string, returnMsg string, discount types.Decimal, extParams string) (enums.FlowStatus, error) {
	...
	default: //结果未知处理
		err := SaveUnkown(deliveryID, source.Format(resultCode), returnMsg)
		return enums.Unknown, err
	}
}

//SaveUnkown 保存未知的发货结果
func SaveUnkown(deliveryID int64, code string, msg string) error {
	row, err := hydra.C.DB().GetRegularDB().Execute(sql.UpdateTradeDeliveryForSaveUnkown, map[string]interface{}{
...
func SaveSuccess(deliveryID int64, discount types.Decimal, params string, code string, msg string) error {
	...
	if row == 0 {
		db.Rollback()
		return fmt.Errorf("修改订单(%d)为发货成功失败", deliveryID)
	}
	db.Commit()
	return nil
}
