...
func Request(ctx hydra.IContext) (r interface{}) {
	...
	if err != nil || product.Len() == 0 {
		return errs.NewError(int(enums.CodeParamNoSetting), fmt.Errorf("商户产品未配置:%v", err))
	}
	...
	flow.NextByFirst(enums.FlowFlagRecvOrderByPID, product.GetInt(sql.FieldPlID), enums.Success,
