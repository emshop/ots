...
func (o *Order) RequestHandle(ctx hydra.IContext) interface{} {
	...
	product, err := mers.GetProductByID(ctx.Request().GetInt(sql.FieldMerProductID))
	if err != nil || product.Len() == 0 {
		return errs.NewError(int(enums.CodeParamNoSetting), "商户产品未配置")
	}

	ctx.Log().Info("3. 查询产品对应的货架配置")
	shelf, err := mers.GetShelfByID(product.GetInt(sql.FieldMerShelfID))
	...
	err = flow.NextByFirst(product.GetInt(sql.FieldPlID), enums.Success,
