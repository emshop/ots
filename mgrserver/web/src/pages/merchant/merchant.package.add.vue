<template>
  <!-- Add Form -->
  <el-dialog title="添加组合商品" width="65%"  :visible.sync="dialogAddVisible">
    <el-form :model="addData" :inline="true" :rules="rules" ref="addForm" label-width="110px">
      <el-form-item label="包名称:" prop="pg_name">
			...
			<el-form-item label="商品编号:" prop="mer_product_id">
				<el-select size="medium" style="width: 100%;"	v-model="addData.mer_product_id"	clearable filterable class="input-cos" placeholder="---请选择---">
					<el-option v-for="(item, index) in merProductID" :key="index" :value="item.value" :label="item.name"></el-option>
				</el-select>
			</el-form-item>
...
export default {
	data() {
		return {
			...
			merProductID:this.$enum.get("merchant_product"),
			...
			rules: {                    //数据验证规则
				...
				mer_product_id: [{ required: true, message: "请输入商品编号", trigger: "blur" }],
	...
	methods: {
		...
		add(formName) {
			this.$refs[formName].validate((valid) => {
				if (valid) {
					this.$http.post("/merchant/package", this.addData, {}, true, true)
						.then(res => {
							this.$refs[formName].resetFields()
							this.dialogAddVisible = false
							this.refresh()
