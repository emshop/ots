...
<el-table-column align="center" width="100" prop="order_id" label="订单编号"></el-table-column>
...
<el-table-column align="center" width="100" prop="create_time" label="创建时间"></el-table-column>
...
export default {
 ...
 methods: {
   ...
   queryData:function(){
     ...
     this.$http.post("/trade/order/query",this.query)
       .then(res => {
         this.dataList.items = res.list;
         this.dataList.count = new Number(res.count);
         this.paging.total = Math.ceil(this.datalist.count / this.paging.ps);
       })
       .catch(err => {
           this.$notify({
             title: "错误",
             message: "网络错误,请稍后再试"+err,
             type: "error",
             offset: 50,
             duration: 2000
           });
