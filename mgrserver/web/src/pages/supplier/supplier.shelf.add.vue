	...
	data() {
      ...
      status: this.$enum.get("status"),
			rules: {                    //数据验证规则
				...
				can_refund: [{ required: true, message: "请输入支持退货", trigger: "blur" }],
				status: [{ required: true, message: "请输入货架状态", trigger: "blur" }],
				limit_count: [{ required: true, message: "请输入单次最大发货数量", trigger: "blur" }],
			},
		}
	},
	props: {
