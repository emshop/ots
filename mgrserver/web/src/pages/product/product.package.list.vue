    ...
    <!-- query start -->
		<div class="panel-body" id="panel-body">
			<el-form ref="form" :inline="true" class="form-inline pull-left">
				...
				<el-form-item>
					<el-select size="medium" v-model="queryData.parent_id"  clearable filterable class="input-cos" placeholder="请选择父级商品">
    ...
    <!-- list start-->
		<el-scrollbar style="height:100%">
			<el-table :data="dataList.items" stripe style="width: 100%" :height="maxHeight" @sort-change="sort">
				...
				<el-table-column   prop="parent_id" label="父级商品" align="center">
					<template slot-scope="scope">
						<span >{{scope.row.parent_id | fltrEnum("product_package")}}</span>
					</template>
				</el-table-column>
...
<script>
import Add from "./product.package.add"
import Edit from "./product.package.edit"
export default {
  components: {
		Add,
		Edit
  ...
  mounted(){
    ...
    query(){
      ...
      let res = this.$http.xpost("/product/package/query",this.$utility.delEmptyProperty(this.queryData))
    ...
    resetForm(formName) {
      ...
      this.$emit("addTab","详情"+val.pg_id,"/product/package/detail",data);
		},
    showAdd(){
      this.$refs.Add.show();
		},
